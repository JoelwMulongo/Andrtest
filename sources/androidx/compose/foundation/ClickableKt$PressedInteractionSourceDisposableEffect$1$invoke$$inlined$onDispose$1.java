...
public final class ClickableKt$PressedInteractionSourceDisposableEffect$1$invoke$$inlined$onDispose$1 implements DisposableEffectResult {
    ...
    public void dispose() {
        ...
        if (oldValue != null) {
            this.$interactionSource$inlined.tryEmit(new PressInteraction.Cancel(oldValue));
            this.$pressedInteraction$inlined.setValue(null);
        }
        for (PressInteraction.Press it : this.$currentKeyPressInteractions$inlined.values()) {
            this.$interactionSource$inlined.tryEmit(new PressInteraction.Cancel(it));
        }

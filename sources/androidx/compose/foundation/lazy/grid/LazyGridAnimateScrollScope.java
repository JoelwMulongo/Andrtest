...
public final class LazyGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        if (this.state.isVertical$foundation_release()) {
            i = IntOffset.m6105getYimpl(item.m953getOffsetnOccac());
        } else {
            i = IntOffset.m6104getXimpl(item.m953getOffsetnOccac());
        }
    ...
    private final int calculateLineAverageMainAxisSize(List<? extends LazyGridItemInfo> visibleItems, boolean isVertical) {
        int i;
        ...
        while (lineStartIndex < visibleItems.size()) {
            ...
            if (currentLine == -1) {
                ...
                while (lineEndIndex < visibleItems.size() && ((Number) lineOf.invoke(Integer.valueOf(lineEndIndex))).intValue() == currentLine) {
                    if (isVertical) {
                        i = IntSize.m6145getHeightimpl(((LazyGridItemInfo) visibleItems.get(lineEndIndex)).m954getSizeYbymL2g());
                    } else {
                        i = IntSize.m6146getWidthimpl(((LazyGridItemInfo) visibleItems.get(lineEndIndex)).m954getSizeYbymL2g());
                    }

...
public final class LazyStaggeredGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        long it = findVisibleItem.m1040getOffsetnOccac();
        return Integer.valueOf(this.state.isVertical$foundation_release() ? IntOffset.m6105getYimpl(it) : IntOffset.m6104getXimpl(it));
    }
    ...
    public float expectedDistanceTo(int index, int targetScrollOffset) {
        List $this$fastSumBy$iv = this.state.getLayoutInfo().getVisibleItemsInfo();
        int sum$iv = 0;
        ...
        for (int index$iv$iv = 0; index$iv$iv < size; index$iv$iv++) {
            ...
            sum$iv += this.state.isVertical$foundation_release() ? IntSize.m6145getHeightimpl(it.m1041getSizeYbymL2g()) : IntSize.m6146getWidthimpl(it.m1041getSizeYbymL2g());
        }
        int averageMainAxisItemSize = sum$iv / ($this$fastSumBy$iv.size() * this.state.getLaneCount$foundation_release());
        ...
        return (((float) (averageMainAxisItemSize * indexesDiff)) + ((float) coercedOffset)) - ((float) getFirstVisibleItemScrollOffset());
    }

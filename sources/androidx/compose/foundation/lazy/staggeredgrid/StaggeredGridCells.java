...
public interface StaggeredGridCells {
    ...
    public static final class Adaptive implements StaggeredGridCells {
        ...
        private Adaptive(float minSize2) {
            ...
            if (!(Dp.m5985compareTo0680j_4(minSize2, Dp.m5986constructorimpl((float) 0)) > 0)) {
                throw new IllegalArgumentException("Failed requirement.".toString());
            }
        }
        ...
        public List<Integer> calculateCrossAxisCellSizes(Density $this$calculateCrossAxisCellSizes, int availableSize, int spacing) {
            Intrinsics.checkNotNullParameter($this$calculateCrossAxisCellSizes, "<this>");
            return LazyStaggeredGridCellsKt.calculateCellsCrossAxisSizeImpl(availableSize, Math.max((availableSize + spacing) / ($this$calculateCrossAxisCellSizes.m5961roundToPx0680j_4(this.minSize) + spacing), 1), spacing);
        }

        public int hashCode() {
            return Dp.m5992hashCodeimpl(this.minSize);
        }

        public boolean equals(Object other) {
            return (other instanceof Adaptive) && Dp.m5991equalsimpl0(this.minSize, ((Adaptive) other).minSize);
        }
    }
}

...
final class SliderKt$RangeSlider$2$onDrag$1$1 extends Lambda implements Function2<Boolean, Float, Unit> {
    ...
    public final void invoke(boolean isStart, float offset) {
        ...
        if (isStart) {
            ...
            offsetRange = RangesKt.rangeTo(RangesKt.coerceIn(this.$rawOffsetStart.getValue().floatValue(), this.$minPx.element, offsetEnd), offsetEnd);
        } else {
            ...
            offsetRange = RangesKt.rangeTo(offsetStart, RangesKt.coerceIn(this.$rawOffsetEnd.getValue().floatValue(), offsetStart, this.$maxPx.element));
        }
